...
export const payment = (items, restaurant) => async (dispatch) => {
  try {
    ...
    // when you upaymenting, it will direct you to the same page you're in
    if (data.url) {
      window.location.href = data.url;
    } else {
      console.log("error")
    }
    // success of the order payment will be handled by stripe itself; we just have to post stripe the data
  } catch (error) {
    dispatch({
