...
const Register = () => {
  ...
  // since user is an object, we are passing the key values
  ...
  useEffect(() => {
    ...
    if(isAuthenticated) {
      // you will not have to click anywhere, if/when you are authenticated, the navigate will take you there(given path)
      navigate("/")
    }
    if(error) {
      alert.error(error)
  ...
  const onChange = (e) => {
    ...
    if(e.target.name === 'avatar') {
      // now we have to handle the file, and fetch them in our database
      const reader = new FileReader()
      // after file is read, onload, as an event listener
      reader.onload = () => {
      ...
      reader.readAsDataURL(e.target.files[0])       //  file reading process; 1st file selects (usually it is the only); and triggers the onload
    } else {
